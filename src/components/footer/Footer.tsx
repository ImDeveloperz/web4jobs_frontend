...
const Footer = () => {
  ...
  return (
    <div className='bg-secondary-color '>
      <div className={`${screenWidth} py-8  mx-auto w-full flex flex-col gap-8`}>
        <div className='flex md:hidden md:justify-start w-full items-center justify-center flex-col gap-2 '>
          <Image alt="logo" layout="intrinsic" src={Text.Footer.logo2} width={124} height={41} />
        </div>
          ...
          <div className='flex flex-col gap-4'>
            ...
            <ul className='text-md w-full  flex flex-col gap-5 font-light leading-3 '>
              {Text.Footer.list1.map((item, index) => (
                <li className='md:text-base text-xs cursor-pointer font-normal underline tracking-wider' key={index}
                onClick={()=>{
                  if(item.name === 'Blogs') {
          ...
          <div className='flex flex-col gap-4'>
            ...
            <ul className='text-md w-full  flex flex-col gap-5 font-light leading-3 '>
              {Text.Footer.list2.map((item, index ) => (
                <li className='md:text-base text-xs w-full cursor-pointer font-normal underline tracking-wider' key={index}>{item}</li>
              ))}
            </ul>
          </div>
          <div className='flex flex-col  gap-4' >
            <h1 className='font-bold md:text-2xl text-lg'>{Text.Footer.title3}</h1>
            <ul className='text-md w-full flex md:items-start items-center flex-col gap-5 font-light leading-3 '>
              {Text.Footer.list3.map((item, index) => (
                <div key={index} className='flex gap-2'>
                  ...
                  <li className='md:text-base  text-xs cursor-pointer decoration-solid md:text-left text-center underline font-normal tracking-wider'  onClick={()=>{router.push(item.link)}} key={index}>{item.name}</li>
                </div>
              ))}
            </ul>
          </div>
        </div>
        <div className='h-[1px] w-full bg-white rounded-lg opacity-70'>
        </div>
